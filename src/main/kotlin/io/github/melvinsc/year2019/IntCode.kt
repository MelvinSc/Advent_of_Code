    ...
    fun eval(program: IntArray, inputs: List<Int> = emptyList()): List<Int> {
        ...
        val OPS = mapOf(
            1 to Op(4) { program, position, modes, _, _ ->
                program[program[position + 3]] =
                    getValue(program, position + 1, modes[0]) + getValue(program, position + 2, modes[1])
                null
            },
            ...
            5 to Op(3) { program, position, modes, _, _ ->
                getValue(program, position + 1, modes[0]).takeIf { it != 0 }
        ...
        while (index in program.indices) {
            ...
            parseOp(rawOp)?.let { (op, modes) ->
                val jump = op.eval(program, index, modes, input, output)
